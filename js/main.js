/**
 * Variavel de tempo;
 * Time variable;
 */
var lasttime,
  /**
   * Variavel canvas;
   * Canvas variable;
   */
  canvas = document.getElementById("canvas"),
  /**
   * Variavel contexto;
   * Context variable;
   */
  ctx = canvas.getContext("2d"),
  /**
   * Animação em andamento;
   * Animation in progress;
   */
  animating = true;

/**
 * Parametros das bolinhas;
 * Parameters of the balls;
 */
var params = {
  /**
   * parametros das bolinhas;
   * ball parameters
   */
  bounce: 0.1,
  /**
   * bolinhas existem;
   * balls exist
   */
  dots: true,
  fric: 1,
  /**
   * distancia inicial das bolinhas;
   * initial distance of the balls;
   */
  initialinset: 500,
  /**
   * linhas existem;
   * lines exist
   */
  lines: true,

  mult: 0.000042,
  /**
   * numero de bolinhas;
   * number of balls
   */
  nump: 20,
  /**
   * Força de puxar;
   * Force to pull
   */
  pull: 25,
  /**
   * Força de empurrar;
   * Force to push
   */
  push: 25,
  /**
   * Velocidade das bolinhas;
   * Speed of the balls;
   */
  visc: 0.05,
  /**
   * Força de atrito;
   * Friction force;
   */
  wfric: 0.5,
  /**
   * Força de gravidade;
   * Gravity force;
   */
  xgrav: 0,
  /**
   * Força de gravidade;
   * Gravity force;
   */
  ygrav: 0,
};

/**
 *
 */
var particles = particle_system(canvas, params);

// https://stackoverflow.com/questions/11381673/detecting-a-mobile-browser
var mobilecheck = function () {
  var check = false;
  (function (a) {
    if (
      /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i.test(
        a
      ) ||
      /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(
        a.substr(0, 4)
      )
    )
      check = true;
  })(navigator.userAgent || navigator.vendor || window.opera);
  return check;
};

var balance = false;
// funcao que inicia a brincadeira
// function init the jock
function init(time) {
  // faz a definição do canvas
  // define the canvas
  fixdim();

  lasttime = new Date().getTime();

  /**
   * Velocidade desejada
   * Desired speed
   */
  var desiredspeed = 500;
  // Verifica se o dispositivo é mobile
  // Check if the device is mobile
  var mobile = mobilecheck();

  // Caso seja mobile define o numero de particulas menor
  // If it is mobile define the number of particles smaller
  if (mobile) {
    params.nump = 30;
    balance = true;
    desiredspeed = 30;
  }

  particles.reset();

  if (mobile) {
    particles.nextframe();
  }

  (function frame() {
    var time = new Date().getTime();
    var dt = time - lasttime;
    lasttime = time;
    // console.log(dt)

    if (balance) {
      if (dt > desiredspeed * 2) {
        params.nump = Math.round(params.nump / 2);
      } else if (dt < desiredspeed && params.nump < 190) {
        params.nump++;
      }
    }

    if (fixdim() || animating) particles.nextframe();
    setTimeout(frame, 5);
  })();

  lasttime = time;
  // requestAnimationFrame(main)
}

/**
 * @returns {boolean} true if the canvas was resized
 */
function fixdim() {
  // Faz a busca das dimensoes da tela
  // Get the dimensions of the screen
  var dirty = dimensions.update();

  // Caso a tela tenha sido redimensionada
  // Faz o redimensionamento do canvas

  // If the screen has been resized
  // Resize the canvas
  if (dirty) {
    canvas.width = dimensions.width * 2;
    canvas.style.width = dimensions.width;
    canvas.height = dimensions.height * 2 - 20;
    canvas.style.height = dimensions.height - 40;
  }
  return dirty;
}

function main(time) {
  document.title = "\u200b";

  // fixdim()

  var dt = time - lasttime;

  // particles.draw()
  // setInterval(function() {
  // particles.nextframe()
  // }, 5)

  lasttime = time;
  requestAnimationFrame(main);
}

// Faz a inicialização do codigo
requestAnimationFrame(init);

function scrollhandle() {
  // faz o comando de skroll
  var doc = document.documentElement;
  var top = (window.pageYOffset || doc.scrollTop) - (doc.clientTop || 0);

  var r = doc.getBoundingClientRect();

  // 1 - (r.bottom - window.innerHeight)/(r.height - window.innerHeight)

  var rect = canvas.getBoundingClientRect();

  var impoerialheight = Math.min(-rect.top, rect.height) / rect.height;

  let coolnewheight = Math.max(
    impoerialheight,
    top / (r.height - window.innerHeight)
  );
  document.querySelector(".down").style.opacity = 1 - coolnewheight;

  params.mult = 0.000042 * (impoerialheight * 3 + 1);
  params.push = 25 * (impoerialheight * 3 + 1);

  animating = rect.bottom > 0;
}

addEventListener("scroll", scrollhandle);
